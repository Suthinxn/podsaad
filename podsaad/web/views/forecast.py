...
def forecast_test():
    ...
    for station_code in stations:
        ...
        forecast_real = target_scaler.inverse_transform(
...
def forecast_bilstm():
    ...
    for station_code in stations:
        ...
        records = list(CollectionClass.objects().order_by("timestamp").limit(LOOKBACK))
        ...
        df_daily = pd.DataFrame(
            [{f: getattr(r, f) for f in FEATURES + ["timestamp"]} for r in records]
        )
        ...
        if df_daily[FEATURES].isnull().any().any():
            df_daily[FEATURES] = (
                df_daily[FEATURES].interpolate(method="linear").ffill().bfill()
            )
...
def forecast_sarimax():
    ...
    for station_code in stations:
        ...
        exog_future = generate_exog_future(
            df_daily, SARIMAX_EXOG_COLS, future_dates, LOOKBACK
        )
        try:
            forecast = sarimax_model.get_forecast(steps=HORIZON, exog=exog_future)
            forecast_mean = forecast.predicted_mean
